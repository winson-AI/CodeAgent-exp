...
- **Single base root + upstream consistency**: resolve `agents_root = <output_dir or ~/.a2c_agents>` once; when `coding-task-adapter` (or any upstream) supplies `agents_root`/`output_root`, use them verbatim and never fall back to a self-chosen base. `output_root = <agents_root>/understand/android-project-analyst[/<understand_subsystem>]`. A base/root that diverges from the upstream-supplied value blocks with `path_mismatch` (see [output-contract.md](output-contract.md) § Path Accuracy Validation).
...
- **Workspace-state discipline**: `analysis-workspace-state` is refreshed after module inventory, each module node group, each module representation, global representation, and SPEC. The ledger MUST maintain **`analysis_todo_list[]`** and **`pipeline_steps[]`** on every refresh. Downstream roles must not consume artifacts that the ledger marks stale; do not dispatch the next step when required todos are `blocked` without a rerun hook.
...
- **Focused-scope invariant**: when `focused_analysis.enabled` is true (adapter `only_understand_*` or explicit partial migration), the requested attention module/feature is a hard contract. Schedule and analyze only `attention_module_ids` / `allowed_source_roots` plus justified shared context. Do not broaden to whole-project analysis, do not inspect out-of-scope modules except for cross-module pointer evidence, and label all global/SPEC outputs as scoped.
...
- **State monitor discipline**: `analysis-workspace-state` MUST refresh `analysis_todo_list[]` and `pipeline_steps[]` after every major group; Leader reads `analysis_status.pipeline_summary.current_step_id` before next dispatch.
- **Downstream trigger discipline**: adherence to [output-contract.md](output-contract.md) is a hard trigger condition for downstream handlers. The Leader must evaluate handoff packages `P0`–`P6`, persist `handoff_gates` in the workspace ledger and `SPEC/verification.md`, and set `run_manifest.json` → `handoff_package`. Downstream workflows (`coding-task-adapter`, `android-to-kmp-migrator`, `kmp-test-validator`) MUST return `blocked` when required package paths are missing, empty, out-of-path, stale, or contract-invalid — they MUST NOT infer from chat or partial summaries.
...
| Focused analysis cannot resolve requested module/feature | STOP; return `blocked` with `scope_gaps`; do not fall back to full-project analysis. |
| Focused node output analyzes out-of-scope roots | Reject as out-of-scope; rerun with `focused_analysis.allowed_source_roots` and `module_scope` restated. |
...
|---|---|
| Whole-project scope on a large monorepo (e.g., > ~50 Gradle modules or > ~5000 source files) | Warn the user; ask to narrow `analysis_scope` unless the request already has focused_analysis. For focused_analysis, keep the requested module/feature and record scoped coverage in `verification.md`. |
